...
namespace PS05 {
    public class PerlinMesh : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            heightMap = generateAlphaHeightMap(generatePerlinHeightLookup(this.meshSize+1, this.step, this.offset), this.meshSize+1);
            // perlinizeMesh();
        ...
        void Update()
        {
            ...
            prevOffset = offset;
            prevMeshSize = meshSize;
            meshLogic.material.SetFloat("_HeightScale", heightScale);
            meshLogic.material.SetTexture("_AlphaHeightMap", heightMap);
        }
        ...
        private static Texture2D generateAlphaHeightMap(float[,] heightMap, int nVerts) {
            Texture2D tex = new Texture2D(nVerts, nVerts);
            for (int i = 0; i < nVerts; i++) {
                for (int j = 0; j < nVerts; j++) {
                    tex.SetPixel(i, j, new Color(0.0f, 0.0f, 0.0f, heightMap[i, j]));
                }
            }
            tex.Apply();
            return tex;
